...
export const shacl2jsonschema = (store: Store, target: string, context: any, title: any = null) => {
  ...
  for (const propQuad of store.getQuads(namedNode(target), sh('property'), null, null)) {
    ...
    for (const subProp of store.getQuads(propQuad.object, sh('node'), null, null)) {
      for (const subPropShape of store.getQuads(
        subProp.object,
        ...
        console.log(
          `Found a sh:node pointing to another NodeShape: ${subPropShape.subject.value.toString()}, generating nested object in the JSON Schema`
        )
