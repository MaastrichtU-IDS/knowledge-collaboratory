  ...
  return (
    <>
      <form>
        <FormSettings>
          ...
          <div style={{width: '100%', textAlign: 'center'}}>
            ...
            <FormControlLabel
              control={<Checkbox checked={state.shaclValidate} onChange={handleShaclValidate} id="shacl-validate" />}
      ...
      {state.nanopubPublished && (
        <Paper
          elevation={4}
          style={{backgroundColor: '#81c784', padding: '16px', marginBottom: '24px', marginTop: '24px'}}
        >
      ...
      {state.errorMessage && (
        ...
        >
          <pre style={{whiteSpace: 'pre-wrap'}}>⚠️&nbsp;&nbsp;{state.errorMessage}</pre>
        </Paper>
      )}
          ...
          {!user.keyfiles_loaded && (
            <>
              <Typography style={{marginBottom: '8px'}}>
                🔑 Before publishing nanopubs, you need to first generate a private/public key pair, and then publish an
                introduction nanopub to link this key pair to your ORCID.
                <br />
                We can automate this process for you, upon clicking the button above we will generate a key pair, and
                publish a nanopublication to link it to your ORCID:
              </Typography>
              ...
              <Typography>
                📤️ Or, if you already have registered a key pair with your ORCID in the nanopublication network, you
                can upload these keys directly:
              </Typography>
              <form
                encType="multipart/form-data"
                action=""
                onSubmit={handleUploadKeys}
      ...
      {state.published_nanopub && (
        <pre style={{whiteSpace: 'pre-wrap'}}>
          <code className="language-turtle">{state.published_nanopub}</code>
        </pre>
      )}
    </>
  );
