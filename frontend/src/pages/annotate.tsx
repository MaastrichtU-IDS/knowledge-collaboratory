...
- associations: <a list of the 5 most important associations between entities in the text, each association is an object with the fields: "subject" for the subject entity, "predicate" for the relation (treats, affects, interacts with, causes, caused by, has evidence), "object" for the object entity>`
  ...
  return (
    <Container className="mainContainer">
      ...
      <Typography variant="body1" style={{textAlign: 'left', margin: theme.spacing(1, 0)}}>
        This service uses extraction models to help you annotate biomedical text using the{' '}
        <a href="https://biolink.github.io/biolink-model/docs/" target="_blank" rel="noopener noreferrer">
          BioLink model
        </a>
        , and standard identifiers resolved using the{' '}
        <a href="https://name-resolution-sri.renci.org/docs" target="_blank" rel="noopener noreferrer">
          NIH NCATS Translator SRI Name Resolution API
        </a>
        , such as MONDO and PubChem. The annotations are represented with the{' '}
        <a href="https://www.w3.org/RDF" target="_blank" rel="noopener noreferrer">
          RDF
        </a>{' '}
        standard, using the{' '}
        <a href="https://vemonet.github.io/tao" target="_blank" rel="noopener noreferrer">
          TAO ontology
        </a>
        , also used by the{' '}
        <a href="http://pubannotation.org/" target="_blank" rel="noopener noreferrer">
          PubAnnotation
        </a>{' '}
        service. Generated annotations can be downloaded in the{' '}
        <a href="https://json-ld.org" target="_blank" rel="noopener noreferrer">
          JSON-LD
        </a>{' '}
        format, or published as{' '}
        <a href="https://nanopub.net" target="_blank" rel="noopener noreferrer">
          Nanopublications
        </a>{' '}
        after login with your ORCID.
      </Typography>
      {/* <Typography variant="body1" style={{textAlign: 'left', margin: theme.spacing(1, 0)}}>
              ...
              <>
                <Typography variant="h5" style={{textAlign: 'center', marginBottom: theme.spacing(3)}}>
                  ...
                  <Tooltip title={<Typography style={{textAlign: 'center'}}>Delete the entity</Typography>}>
                    <IconButton
                      onClick={() => handleRemoveEntity(state.tagSelected.index)}
                      style={{marginLeft: theme.spacing(1), alignContent: 'right'}}
                      color="default"
                    >
                      <RemoveIcon />
                    </IconButton>
                ...
                {state.templateSelected !== 'Plain RDF' &&
                  state.tagSelected.props &&
                  state.tagSelected.props.map((prop: any, pindex: number) => {
                    return (
                      <Grid
                        container
                        spacing={2}
                        key={'prop:' + prop + pindex}
                        style={{marginLeft: theme.spacing(5), marginBottom: theme.spacing(1)}}
                      >
                        <Grid item xs={5}>
                          <AutocompleteEntity
                            label="Property"
                        ...
                        <Grid item xs={1}>
                          <Tooltip title={<Typography style={{textAlign: 'center'}}>Delete the statement</Typography>}>
                            <IconButton
                              onClick={() => handleRemoveEntityProp(state.tagSelected, pindex)}
                              color="default"
                            >
                              <RemoveIcon />
                            </IconButton>
      ...
      {state.extractClicked && (
        <>
          <Typography variant="body1" style={{textAlign: 'center', marginBottom: theme.spacing(2)}}>
            💡 You can edit entities by clicking on their tag, or add new entities by highlighting the text
            corresponding to the entity. Potential identifiers are automatically retrieved for the highlighted text.
          </Typography>
      ...
      {state.entitiesList.length > 0 && (
        <>
          <Typography variant="body1" style={{marginBottom: theme.spacing(2), marginTop: theme.spacing(6)}}>
            2. Define the statements that represent the assertions made in the text, you can add properties to provide
            more context:
          </Typography>
